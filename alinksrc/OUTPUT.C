...
void BuildPEResources(long sectNum,PUCHAR objectTable)
{
    ...
    for(i=1;i<rescount;i++)
    {
        ...
        for(j=i-1;j>=0;j--)
        {
            if(resource[j].typename_x)
            {
                if(!curres.typename_x) break;
                if(wstricmp(curres.typename_x, resource[j].typename_x)>0) break;
                if(wstricmp(curres.typename_x, resource[j].typename_x)==0)
                {
                    if(resource[j].name)
                    {
            ...
            {
                if(!curres.typename_x)
                {
                    if(curres.typeid_x > resource[j].typeid_x) break;
                    if(curres.typeid_x == resource[j].typeid_x)
                    {
                        if(resource[j].name)
                        {
                            if(!curres.name) break;
    ...
    {
        if(resource[i].typename_x)
        {
            nameSize+=2+2*wstrlen(resource[i].typename_x);
	}
        if(resource[i].name)
        {
    ...
    for(i=0;i<rescount;i++)
    {
        if(!resource[i].typename_x) break;
        if((i>0) && !wstricmp(resource[i].typename_x, resource[i-1].typename_x))
        {
            if(resource[i].name)
            {
    ...
    for(;i<rescount;i++)
    {
        if((i>0) && !resource[i-1].typename_x && (resource[i].typeid_x == resource[i-1].typeid_x))
        {
            if(resource[i].name)
            {
                if(wstricmp(resource[i].name, resource[i-1].name)==0)
    ...
    for(i=0;i<rescount;i++)
    {
        if(!(resource[i].typename_x && curTypeName &&
            !wstricmp(resource[i].typename_x, curTypeName))
            && !(!resource[i].typename_x && !curTypeName && curTypeId==resource[i].typeid_x))
        {
            if(resource[i].typename_x)
            {
                ...
                curTypeName=resource[i].typename_x;
            ...
            for(j=i;j<rescount;j++)
            {
                if(resource[i].typename_x)
                {
                    if(!resource[j].typename_x) break;
                    if(wstricmp(resource[i].typename_x, resource[j].typename_x)!=0) break;
                }
                else
                {
                    if(resource[j].typeid_x != resource[i].typeid_x) break;
                }
                if(resource[j].name)
                {
