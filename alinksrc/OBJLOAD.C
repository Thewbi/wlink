...
void loadres(FILE *f)
{
	...
	{
	    resource[rescount].typename_x=NULL;
	    resource[rescount].typeid_x=hdr[i+2]+256*hdr[i+3];
	    i+=4;
	}
	else
	{
	    ...
	    resource[rescount].typename_x=(PUCHAR)checkMalloc(j-i+2);
	    memcpy(resource[rescount].typename_x,hdr+i,j-i+2);
	    i=j+5;
	    i&=0xfffffffc;
	}
	if(i>hdrsize)
